...
状态含义：`verified-unit` 是单层契约测试，`verified-integration` 是真实
...
| C4 | cancel 响应丢失且订单仍 open；GetOrder 后同 ID cancel | 只解冻一次 | `partial` | `frontend/src/views/Trade.vue::reconcilePendingWrite` 与 `trading/rpc/server/server_test.go::TestCancelOrderIdempotencyPrecedesClosedStateCheck`；缺响应丢失 E2E |
...
| C6 | cancel 响应丢失但订单已终态；读取 GetOrder | 不再发第二次撤单 | `partial` | `frontend/src/trading/pending-write.test.ts` 只证明 pure predicate 能识别终态；尚无 Trade.vue/API spy 证明不会再次调用 cancel |
| C7 | fund 已记账但响应丢失；同 ID fund | ledger 只有一组分录，余额只增加一次 | `partial` | 后端幂等/持久化由 `trading/exchange/exchange_test.go::TestIdempotencyAndConcurrentRetry` 与 `trading/store/postgres/store_integration_test.go::TestPostgresEventSnapshotOutboxAndRecovery` 分层覆盖；没有 response-loss + same-ID fund replay 的 PostgreSQL E2E，也没有 fund request/ledger 查询接口 |
| C8 | 另一个 actor 账户打开 pending；点击 reconcile | 必须拒绝跨 actor 账户核对，fund target 不与 actor 混淆 | `planned/environment-pending` | `Trade.vue` 有 actor guard，但没有跨账户 reconcile 自动测试；服务端 fund 幂等 subject 是目标 `account_id` |
| C9 | 浏览器刷新；原 actor 重新登录并 reconcile | operation/request/payload 不丢 | `partial` | `frontend/src/trading/pending-write.test.ts` 只测试 sessionStorage JSON parser；没有真实 browser refresh/re-login 测试 |
| C10 | 502/503/504；保持 unknown，不自动写第二次 | BFF/客户端只有一次写调用 | `partial` | `frontend/src/api/trading.test.ts` 只覆盖 submit fetch failure；没有 cancel、fund 与 BFF 故障链 |
